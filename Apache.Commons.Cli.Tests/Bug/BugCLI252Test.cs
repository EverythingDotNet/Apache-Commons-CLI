...
namespace Apache.Commons.Cli.Bug {
    ...
    public class BugCLI252Test {
        ...
        [Fact]
        public void ExactOptionNameMatchTest() {
            new DefaultParser().Parse(GetOptions(), new string[]{"--prefix"});
        }


        [Fact]
        public void AmbiquousOptionNameTest() {
            Assert.Throws<AmbiguousOptionException>(() => new DefaultParser().Parse(GetOptions(), new string[] { "--pref" }));
        }
